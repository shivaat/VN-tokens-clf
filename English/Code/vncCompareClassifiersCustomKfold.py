...
seqs = []
# We don't consider the sequences tagged as Q in the dataset
for s in sents:
          if s[2] in ['L', 'I'] and getWords(s):
                    w1,w2,w3,w4 = getWords(s)
                    seq = ([w1,w2,w3,w4], s[1], s[2])
                    seqs.append(seq)
...
cv_results = typeaware_val_multiClassifier(classifiers, X, y, seqs)
...
cv_res.to_csv('VNC_clf_typeaware_res')
